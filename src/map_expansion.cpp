/// \file
/// \brief This node reads in the robot's local SLAM map and creates a larger
/// map to be used for the map_merge node
///
/// PUBLISHES:
...
int main(int argc, char* argv[])
{
  ...
  while (ros::ok())
  {
    if ((slam0_map.data.size() != 0) || (slam1_map.data.size() != 0) || slam0_map.info.origin.position.x != 0 ||
        slam1_map.info.origin.position.x != 0)
    {
      ...
      // Determine how much space to fill in with unknown cells bewteen bottom
      // of the new sized map and the original slam map
      const size_t bottom0_width_ =
          (slam0_map.info.origin.position.x - new_tb3_0_map.info.origin.position.x) / new_tb3_0_map.info.resolution;
      const size_t bottom1_width_ =
          (slam1_map.info.origin.position.x - new_tb3_1_map.info.origin.position.x) / new_tb3_1_map.info.resolution;

      const size_t bottom0_height_ =
          (slam0_map.info.origin.position.y - new_tb3_0_map.info.origin.position.y) / new_tb3_0_map.info.resolution;
      const size_t bottom1_height_ =
          (slam1_map.info.origin.position.y - new_tb3_1_map.info.origin.position.y) / new_tb3_1_map.info.resolution;

      /////////////////////////////////////////////////////////////////////////////
      // For the frist turtlebot (tb3_0)
      ////////////////////////////////////////////////////////////////////////////
      // Map starts loading in from origin, which is the bottom right corner
      // (for the deault orientation in rviz) From the origin, the row
      // components corresponds with width (+ x-dir which is up and + y-dir is
      // to the left) Fill in the all new cells in the new map with unknowns
      // (-1)

      int c0 = 0;  // start a counter for map0

      // Fill in the space between start of the new map to the start of the
      // local SLAM map with -1s (for the default orientation in Rviz, this is
      // the space to the right of the SLAM map)
      for (int i = 0; i < new_tb3_0_map.info.width * bottom0_height_; i++)
      {
        new_tb3_0_map.data.push_back(-1);
      }

      // Fill in the spaces on either side of the local SLAM map with -1s, but
      // dont replace the current values from the local SLAM map
      for (int item_counter = 0; item_counter < slam0_map.info.height; item_counter++)
      {
        // For all new cells between the new starting width and the original
        // SLAM starting width, fill with -1s (for the default orientation in
        // Rviz, this is the space below the SLAM map)
        ...
        // For all new cells between the new ending width and the original SLAM
        // end width, fill with -1s (for the default orientation in Rviz, this
        // is the space above the SLAM map)
      ...
      // Fill in the space between the end of the original SLAM map to the end
      // of the new map with -1s (for the default orientation in Rviz, this is
      // the space to the left of the SLAM map)
